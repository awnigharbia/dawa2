...
class GeoFireCollectionRef {
  ...
  /// set a geo point with [latitude] and [longitude] using [field] as the object  to the document with [id]
  ...
  /// [field] specifies the name of the  in the document
  Stream<List<DistanceDocSnapshot>> within({
    ...
    Iterable<Stream<List<DistanceDocSnapshot>>> queries = area.map((hash) {
      final tempQuery = _queryPoint(hash, field, limit);
