...
GeoFlutterFire is an open-source library that allows you to store and query a set of s based on their geographic location. At its heart, GeoFlutterFire simply stores locations with string s. Its main benefit, however, is the possibility of retrieving only those s within a given geographic area - all in realtime.
